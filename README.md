...
- 自动汉化：可通过 OpenAI 兼容接口生成中文用途和推荐中文简称，支持单项推荐与批量补全。
- 预览应用：AI 结果不会直接覆盖现有内容，可分别勾选简称和用途后再应用。
...
中文别名、用途和收藏状态保存在应用自己的配置目录中，不会写回原始 Skill。使用 AI 汉化时，只会向用户配置的接口发送 Skill 调用名、原始名称和原始说明；API 密钥保存在 Windows 凭据管理器中，不写入应用配置文件。未使用 AI 汉化时，应用不会上传数据。
